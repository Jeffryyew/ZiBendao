...
export async function GET() {
  ...
  let dbStatus: { ok: boolean; userCount?: number; accountCount?: number; error?: string; accountError?: string } = { ok: false };
  try {
  ...
  // Test Account table (required for Google OAuth)
  try {
    const accountCount = await prisma.account.count();
    dbStatus.accountCount = accountCount;
  } catch (e) {
    dbStatus.accountError = String(e);
  }

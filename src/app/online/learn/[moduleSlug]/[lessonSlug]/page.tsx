  ...
  return (
    ...
    >
      ...
      <div className="flex-1">
        {lesson.type === "STORY" && (
          <StoryLesson lesson={lesson} mod={mod} onComplete={handleComplete} completed={completed} />
        )}
        {lesson.type === "QUIZ" && (
          <QuizLesson lesson={lesson} mod={mod} onComplete={handleComplete} completed={completed} />
        )}
        {lesson.type === "SIMULATION" && (
          <SimulationLesson lesson={lesson} mod={mod} onComplete={handleComplete} completed={completed} />
        )}
        {lesson.type === "REFLECTION" && (
          <ReflectionLesson lesson={lesson} mod={mod} onComplete={handleComplete} completed={completed} />
        )}
      </div>
  ...
  return (
    <div className="flex flex-col h-full max-w-2xl mx-auto px-4 py-6">
        ...
        <div className="flex-1">
          ...
          {current.highlight && (
            <div
              ...
              style={{ background: `${color}15`, border: `1px solid ${color}33`, color, fontSize: "0.85rem", fontWeight: 700 }}
            >
              {current.highlight}
            </div>
          )}
      ...
      <div className={`flex gap-3 mt-8 ${completed ? "pb-32" : ""}`}>
        ...
        <button
          onClick={() => { if (isLast) { onComplete(); } else { setSlideIdx((i) => i + 1); } }}
          className="flex-1 py-3 rounded-xl font-bold text-sm transition-all active:scale-95"
          style={{ background: `linear-gradient(135deg, ${color}22 0%, ${color}11 100%)`, color, border: `1px solid ${color}33` }}
        >
          {isLast ? "完成阅读" : "下一页 →"}
        </button>
      </div>
...
function QuizLesson({
  ...
  const handleNext = () => {
    if (isLastQ) { onComplete(); }
    else { setQIdx((i) => i + 1); setSelected(null); setAnswered(false); setCorrect(false); }
  };
      ...
      <div className="space-y-3 mb-8">
        {q.options.map((opt) => {
          ...
          if (answered && opt.correct) {
            bg = "rgba(34,197,94,0.12)"; border = "1px solid rgba(34,197,94,0.4)"; textColor = "#86efac";
          } else if (answered && isSelected && !opt.correct) {
            bg = "rgba(239,68,68,0.12)"; border = "1px solid rgba(239,68,68,0.4)"; textColor = "#fca5a5";
          } else if (isSelected) {
            bg = `${color}15`; border = `1px solid ${color}44`; textColor = color;
          }
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-6">
      ...
      {lesson.reflectionPrompt && (
        <div className="mb-4">
          <div className="text-xs font-mono mb-2" style={{ color: "rgba(255,255,255,0.35)" }}>深度思考</div>
      ...
      <textarea
        ...
        style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", color: "#FFFFFF", lineHeight: 1.6 }}
      />

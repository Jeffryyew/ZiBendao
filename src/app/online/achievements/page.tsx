    ...
    >
      <div
        ...
        style={{
          ...
          borderRadius: 20, padding: "32px 28px", maxWidth: 320, width: "90%", textAlign: "center",
        }}
      >
        ...
        <div style={{
          fontSize: "0.7rem", padding: "6px 14px", borderRadius: 999, display: "inline-block",
        ...
        <div onClick={onClose} style={{ marginTop: 20, fontSize: "0.7rem", color: "rgba(255,255,255,0.2)", cursor: "pointer" }}>
          关闭
        </div>
      </div>
    </div>
...
function AnimatedBadgeCard({
  badge, state, isNew, animDelay, onAnimDone, onClick,
}: {
  badge: Badge; state: BadgeState; isNew: boolean; animDelay: number;
  onAnimDone: () => void; onClick: () => void;
}) {
  ...
  useEffect(() => {
    ...
    const t3 = setTimeout(() => { setPhase("done"); onAnimDone(); }, animDelay + 1400);
  ...
  return (
    ...
    >
      <BadgeIcon badge={badge} state={state} size="md" isNew={state === "unlocked_new" && phase !== "done"} />
    </div>
  );
}
...
export default function AchievementsPage() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "linear-gradient(180deg, #07071a 0%, #050510 100%)", color: "#fff", paddingBottom: 80 }}>
      <style>{`
      ...
      <div style={{
        ...
        <div style={{ flex: 1, textAlign: "center" }}>
          ...
          <div style={{ fontSize: "0.65rem", color: "rgba(255,255,255,0.35)" }}>
            {earnedOnline}/{ONLINE_BADGES.length} 线上 · 共 {ALL_BADGES.filter(b => states[b.id] !== "locked").length}/{ALL_BADGES.length} 枚
          </div>
        </div>
        <div style={{ width: 40 }} />
      </div>
      ...
      <div style={{ padding: "24px 20px 0", maxWidth: 480, margin: "0 auto" }}>
        ...
        <div style={{ animation: "sectionFadeIn 0.5s ease both", marginBottom: 40 }}>
          {sectionTitle("线上成长徽章", `每完成 2 关解锁 1 枚 · 共 ${ONLINE_BADGES.length} 枚`)}
          <div style={{ marginBottom: 20 }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
              <span style={{ fontSize: "0.65rem", color: "rgba(255,255,255,0.3)" }}>进度</span>
              <span style={{ fontSize: "0.65rem", color: "#F59E0B" }}>{earnedOnline} / {ONLINE_BADGES.length}</span>
            </div>
            <div style={{ height: 4, borderRadius: 2, background: "rgba(255,255,255,0.08)" }}>
              <div style={{
                height: "100%", width: `${(earnedOnline / ONLINE_BADGES.length) * 100}%`, borderRadius: 2,
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "16px 8px" }}>
            {ONLINE_BADGES.map(badge => {
              ...
              return (
                <AnimatedBadgeCard
                  key={badge.id} badge={badge} state={st} isNew={isNew}
                  animDelay={getAnimDelay(badge)} onAnimDone={() => handleAnimDone(badge.id)}
        ...
        <div style={{ animation: "sectionFadeIn 0.5s ease 0.3s both" }}>
          ...
          <div
            style={{
              ...
              background: states["ultimate"] !== "locked" ? "linear-gradient(145deg, #1c0a00, #3b1f00)" : "rgba(255,255,255,0.03)",
              border: states["ultimate"] !== "locked" ? "1px solid #F59E0B44" : "1px solid rgba(255,255,255,0.06)",
            ...
            <div style={{ marginTop: 16, textAlign: "center" }}>
              ...
              {states["ultimate"] === "locked" && (
                <div style={{ marginTop: 12, display: "flex", justifyContent: "center", gap: 6, flexWrap: "wrap" }}>
                  {[
                    { label: `线上 ${ONLINE_BADGES.length} 关`, done: earnedOnline >= ONLINE_BADGES.length },
      ...
      {selected && (
        <BadgeDetail badge={selected} state={states[selected.id] || "locked"} onClose={() => setSelected(null)} />
      )}
    </div>
  );
}
